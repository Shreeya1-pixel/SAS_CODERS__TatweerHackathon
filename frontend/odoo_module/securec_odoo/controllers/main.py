    ...
    def attack_lab_run(self, input_text='', module='AttackLab', **kwargs):
        ...
        try:
            ...
            logs = request.env['securec.log'].sudo().search(
                ...
                'timestamp', 'module', 'risk_score', 'decision',
                'detected_patterns', 'explanation',
            ...
            jira_alert = None
            if sec_log_id:
                row = request.env['securec.log'].sudo().browse(sec_log_id)
                if row.exists() and row.jira_ticket_id:
                    jira_alert = {
                        'ticket_id': row.jira_ticket_id,
