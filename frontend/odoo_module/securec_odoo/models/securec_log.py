...
class SafeOLog(models.Model):
    ...
    def create(self, vals_list):
        ...
        for record in records:
            if record.decision == 'block' and record.risk_score >= 0.70:
                record._try_create_jira_ticket()
                record._send_odoo_notification()
        return records

    def _try_create_jira_ticket(self):
        """Auto-create a Jira alert ticket when ERP input is BLOCKed."""
        ...
        if not all([jira_url, jira_user, jira_token]):
            _logger.info(
                "SafeO: Jira skipped for log %s — configure securec.jira_url, jira_user, jira_token in Settings",
                self.id,
            )
            return

        if self.jira_ticket_id:
            return

        try:
            policy_note = ""
            if self.policy_region:
                policy_note = f"Policy Region: {self.policy_region}\n"
            if self.policy_decision_reason:
                policy_note += f"Policy Reason: {self.policy_decision_reason}\n"

            payload = {
                "fields": {
                    "project": {"key": jira_project},
                    "summary": f"[SafeO] BLOCKED ERP input — {self.module} module",
                    "description": {
                        "type": "doc",
                        "version": 1,
                        "content": [{
                            ...
                            "content": [{
                                "type": "text",
                                "text": (
                                    f"SafeO blocked a high-risk ERP input and created this alert.\n\n"
