...
async def _run_scan(input_text: str, context: ScanContext) -> Dict[str, Any]:
    ...
    # Policy check — includes UAE law articles when jurisdiction=UAE
    from ..agents.policy_agent import PolicyAgent as _PolicyAgent
    _policy_result = _PolicyAgent().check(
        payload=input_text,
        context={"jurisdiction": context.jurisdiction or "Global"},
        normalised_text=ml_norm,
        risk_score=final_score,
        decision=decision.lower(),
    )
    result["policy"] = _policy_result

    from ..utils.xai_breakdown import build_xai_breakdown
    result["xai_audit"] = build_xai_breakdown(
        payload=input_text,
        patterns=patterns,
        explanations=explanations,
        meta=meta,
    ...
    if decision == "BLOCK":
        ...
        asyncio.create_task(
            run_investigation(
                ...
                patterns=patterns,
                meta=meta,
                context={"user_id": user_id, "jurisdiction": context.jurisdiction or "Global"},
                behavior_score=beh,
            )
        )
